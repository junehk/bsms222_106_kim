...
$$
{\rm Prob}(+~|~D = 1)=0.99,~{\rm Prob}(-~|~D=0)=0.99
$$
 

with $+$ meaning a positive test and $D$ representing if you actually have the disease (1) or not (0).

Suppose we select a random person and they test positive. What is the probability that they have the disease? We write this as ${\rm Prob}(D=1~|~+)$? The cystic fibrosis rate is 1 in 3,900 which implies that ${\rm Prob}(D=1)=0.00025$. To answer this question, we will use Bayes theorem, which in general tells us that:

$$
{\rm Pr}(A~|~B)={{{\rm Pr}(B~|~A){\rm Pr}(A)}\over{\rm Pr}(B)}
$$
...
\rm Pr(+)}} \\
&={{
{\rm Pr}(+~|~D=1) \cdot P(D=1)
}\over{
{\rm Pr}(+~|~D=1) \cdot P(D=1)+{\rm Pr}(+~|~D=0){\rm Pr}(D=0)
}}
\end{aligned}
$$
...
$$
\sqrt{.450(1-.450)\over 20}=.111
$$

This means that our confidence interval is $.450 - .222$ to $.450 + .200$ or $.228$ to $.672$.
...
Based on the plots we showed earlier, we assume that  
p
  has a normal distribution. With expected value .270 and standard error 0.027.
...
\begin{aligned}
{\rm E}(p ~|~ Y=.450)
&= B ~\times~ .275+(1-B) ~\times~.450 \\
&= .275+(1-B)(.450-.275) \\
B &= {.111^2 \over .111^2 + .027^2}=0.944 \\
...
$$
{\rm SE}(p ~|~ y)^2 
= {1 \over 1/\sigma^2 + 1/\tau ^2}
= {1 \over 1/.111^2 + 1/.027^2}
= 0.00069
$$
...
## 16.7 Exercises
1\. In 1999, in England, Sally Clark[^58] was found guilty of the murder of two of her sons. Both infants were found dead in the morning, one in 1996 and another in 1998. In both cases, she claimed the cause of death was sudden infant death syndrome (SIDS). No evidence of physical harm was found on the two infants so the main piece of evidence against her was the testimony of Professor Sir Roy Meadow, who testified that the chances of two infants dying of SIDS was 1 in 73 million. He arrived at this figure by finding that the rate of SIDS was 1 in 8,500 and then calculating that the chance of two SIDS cases was 8,500 $×$ 8,500 $≈$ 73 million. Which of the following do you agree with?

[^58]:https://en.wikipedia.org/wiki/Sally_Clark
  ...
  ${\rm Pr(second~case~of~SIDS~∣~first~case~ of~ SID})<{\rm Pr(first~case~of~SIDS)}$.
  b. Nothing. The multiplication rule always applies in this way:  
  ${\rm Pr}(A{\rm ~and~}B)={\rm Pr}(A){\rm Pr}(B)$  
  c. Sir Meadow is an expert and we should trust his calculations.
  d. Numbers don’t lie.
  
  answer : 추가설명 필요. 이해가 안됨.
  
2\. Let’s assume that there is in fact a genetic component to SIDS and the probability of  
${\rm Pr(second~case~of~SIDS~∣~first~case~ of~ SID})<{\rm Pr(first~case~of~SIDS)}=1/100$, is much higher than 1 in 8,500. What is the probability of both of her sons dying of SIDS?
...
$$
\rm {{
Pr(children~with~no~evidence~of~physical~harm~|~mother~is~murderer) \times Pr(mother~is~murderer)
}\over{
Pr(children~with~no~evidence~of~physical~harm)
}}
$$
...
with A = *two of her children are found dead with no evidence of physical harm* and B = *a mother is a son-murdering psychopath* = 0.50. Assume that the rate of son-murdering psychopaths mothers is 1 in 1,000,000. According to Bayes’ theorem, what is the probability of${\rm Pr}(B~∣~A)$?
...
7\. Now assume a Bayesian model that sets the prior distribution for Florida’s election night spread $d$ to be Normal with expected value $μ$ and standard deviation $τ$. What are the interpretations of $μ$ and $τ$?
  ...
  여러 선거에서의 평균 spread $μ$, 표준편차 $τ$ 이기 때문에, 자의적인 수는 아님.
...
ci <- x + c(-qnorm(0.975)*sd, qnorm(0.975)*sd)
ci
```
...
p <- sapply(tau_var, f_x)
plot(tau_var, p)
```
> 이 그래프가 나타내는 의미가 무엇일까?
...
\begin{aligned}
&d∼N(μ,τ^2)~{\rm
describes~our~best~guess~had~we~not~seen~any~polling~data
}\\
...
#> [1] 0.0160 0.0401
```
The posterior probability ${\rm Pr}(d>0~∣~\bar X)$ can be computed like this:
...
> 이 부분 완전히 이해가 안간다. 설명 필요.  Pr(d>0 | bar X) 이것이 무엇을 나타내는 말인지 이해가 안감.
...
f1 <- function(N){
  ...
  sapply(1:I, function(i){
    ...
    gather(polster, spread) %>%
    mutate("N"=N)
}

# out = list() # 빈 리스트를 만드는 것
# out[[n]] = fx(n)
# list 활용법을 알아두자
...
m %>% ggplot(aes(x=polster, y = spread)) +
  geom_point() +
  facet_wrap(~N, ncol = 3) + 
  coord_flip()
```

> **cowplot**이용하는 방법! `n=20` 짜리 ggplot 을 `p1` 으로 저장. `n=2000` 짜리 ggplot 을 `p2`, `n=200000` 짜리 plot 을 `p3`로 저장하자.  
> **cowplot** 을 이용해서, `plot_grid(p1, p2, p3, ncol = 3)` 하면 정말 멋진 그림이 나올 것이다.

[](https://rafalab.github.io/dsbook/book_files/figure-html/simulated-data-without-bias-1.png)

> `gather` 에서 빼고 싶은 key 가 있으면, 명령어 `- V2` 이런식으로 하면 된다.
...
$$
\bar X = d + b+ {1 \over N}\sum^N_{i=1}X_i
$$
,
