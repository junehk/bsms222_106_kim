...
> ```{bash} 하면 터미널에서 실행된다고 한다.
> ~.gz 파일을 한 번 풀어서 확인해보려고 했으나, 이 명령어가 잘 적용이 안되었다 ㅠㅠ
...
> readr package를 쓰는 이유 : 큰 파일 읽는 때 불러읽는 속도가 빠르다. 이 package의 함수 read_delim : de limiter
> delim을 할때 예전엔 ','로 했지만 요즘은 탭 '\t'을 이용.
> skip = 5 5줄을 생략한다. 왜냐? 이 데이타 파일의 앞의 5줄엔 ##로 컴퓨터에겐 불필요한 정보가 있다.
> progress = F : 진행상황 굳이 안볼꺼다.
> col_names = F : 이 데이터엔 column name이 지정되어있지 않다. 이걸 만일 T로 하면 맨 위의 column이 자동으로 column name으로 지정되기 때문에 이제 직접 이름을 정해줄꺼다.
...
> X1 : 염색체 번호
> X2 :
> X3 : 유전자 정보 : 유전자마다 transcript가 여러개 있다 : alternative splicing
> X4,5 : loci 시작위치, 끝나는 위치 : interval 정보가 있는데, chromosome 이 겹치면 이 interval 이 겹칠 가능성도 있다.
> X7 : strand. 어느 strand에 있는가? +? -?
...
> 사실 이위의 사이트 가면 column 정보를 알 수 있다.
...
> 위의 사이트에서 찾아낸 정보들. 단, 우리 코딩인생에 이제 띄어쓰기와 하이픈은 없다. 무조건 under_score!!
...
> 17만 개가 넘어가는 정보가 있는데 왜? exon intron 다 들어있어.
...
> d1 보면 gene 만 6만개? 왜이렇게 많지.
> info 정보를 보면 : "transcribed_unproceessed_pseudogene" 이 있다. 전사는 일어나지만, 단백질은 만들지 않는 gene도 존재한다.
> 6만개 중에 coding gene 도 있찌만 ,pseudo gene도 있고, ig gene도 있다.

```{r}
#gene 만 저장한 d1 파일을 저장하는 방법
write.table(d1, 'table.gene_genecode.20190918.txt', sep='\t', quote=F, row.names = F, col.names = T)
```

> 'grap()' 을 이용해서 partial matching 할 수 있다.
> 물론 손쉽게 뽑는 패키지가 있긴 하지만, 나중에 배울 것.
