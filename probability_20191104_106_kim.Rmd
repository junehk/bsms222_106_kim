...
X
 , can be 1,2,3,4,5 or 6. The probability of 4 is defined as:
...
x
  ...
  is not defined, there is a theoretical definition that has a similar interpretation. The probability density at  
x
...
x
  ...
  as a curve for which the area under that curve up to the value  
a
...
6. To see the math that explains why the answers to questions 3, 4, and 5 are the same, suppose we have a random variable with average  
m
  and standard error  
s
 . Suppose we ask the probability of  
X
  being smaller or equal to  
a
...
The quantity on the left is a standard normal random variable. It has an average of 0 and a standard error of 1. We will call it  
Z
 :
...
≤
a
  is the same as the probability of a standard normal variable being less than  
(
a
−
m
