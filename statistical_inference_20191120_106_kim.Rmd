...
> **Spread**: 그래프가 평균을 중심으로 퍼진 정도  
> **MoE** = Margin of origin, 오차범위(허용오차)
...
> **p-Value**: 유의확률(significance probablility)
...
take_poll(25)
```
> `take_pull`함수는 매번 다른 결과를 보여준다. `set.seed`를 이용하면 비슷한 결과를 줄 것 같다.
...
$(1-0) \sqrt{p(1-p)}=\sqrt{p(1-p)}$ 여기서 왜 $(1-0)$인지 저번 챕터에서 잘 이해가 안되었다.

$${\rm SE}(\bar X) = \sqrt{p(1-p)/N}$$
...
$$
{\rm E}(\bar X) = p,~\bar X = 1/N \times \sum^N_{i=1}X_i, \\
{\rm Sum~of~random~variables~}S=N\times \bar X,\\
\begin{aligned}
{\rm E}(d) &= {\rm E}(2\bar X - 1) \\
&=2{\rm E}(\bar X) - 1 \\
&=2p-1
\end{aligned}
$$
...
$$
\begin{aligned}
{\rm SE}(d) &= {\rm SE}(2 \bar X -1)\\
&=2~\times {\rm SE}(\bar X) -1\\
&= 2 \sqrt{p(1-p)/N} -1
\end{aligned}
$$
...
p=0.45, ~N=25~이면,\\ 
\begin{aligned}
{\rm SE}(2\bar X - 1) &= 2 \sqrt{p(1-p)/N} -1 \\
&= 2 \sqrt{{0.45 \times (0.55)\over 25}} -1\\
&= -0.8010025
\end{aligned}
$$

```{r}
2*sqrt(0.45*0.55/25)-1
```
...
$${\rm Pr}(|\bar X - p | \leq .01)$$
 
which is the same as:

$${\rm Pr}(\bar X \leq p + .01) - {\rm Pr}(\bar X \leq p -.01)$$

Can we answer this question? We can use the mathematical trick we learned in the previous chapter. Subtract the expected value and divide by the standard error to get a standard normal random variable, call it $Z$, on the left. Since $p$ is the expected value and ${\rm SE}(\bar X) = \sqrt{p(1-p)/N}$ is the standard error we get:

$${\rm Pr} \left( Z \leq {.01 \over {\rm SE}(\bar X)} \right) - {\rm Pr} \left( Z \leq -{.01 \over {\rm SE}(\bar X)} \right)$$
 

One problem we have is that since we don’t know   $p$, we don’t know ${\rm SE}(\bar X)$. But it turns out that the CLT still works if we estimate the standard error by using $\bar X$ in place of $p$. We say that we *plug-in* the estimate. Our estimate of the standard error is therefore:

$${\rm \hat {SE}}(\bar X) = \sqrt{\bar X (1 - \bar X ) / N}$$
...
Now we continue with our calculation, but dividing by ${\rm \hat {SE}}(\bar X) = \sqrt{\bar X (1 - \bar X ) / N}$ instead. In our first sample we had 12 blue and 13 red so $\bar X = 0.48$ and our estimate of standard error is:
...
$${\rm Pr}(Z \leq 1.96~{\rm SE}(\bar X)/{\rm SE}(\bar X))~-~{\rm Pr}(Z \leq - 1.96~{\rm SE}(\bar X)/{\rm SE}(\bar X))$$
which is:

$${\rm Pr}(Z \leq 1.96) - {\rm Pr}(Z \leq -1.96)$$
...
From the table above, we see that typical sample sizes range from 700 to 3500. To see how this gives us a much more practical result, notice that if we had obtained a $\bar X$=0.48 with a sample size of 2,000, our standard error ${\rm \hat{SE}}(\bar X)$ would have been 0.011. So our result is an estimate of 48% with a margin of error of 2%. In this case, the result is much more informative and would make us think that there are more red balls than blue. Keep in mind, however, that this is hypothetical. We did not take a poll of 2,000 since we don’t want to ruin the competition.
...
```{r}
# 위의 histrogram qqplot 을 어떻게 만들까?
...
# class(x_hat)
# [1] "numeric # ggplot 쓰려면 data frame 또는 tibble 으로 만들어야지
...
# column이 x_hat 하나 뿐이기 떄문에, data.frame(x_hat = x_hat) 이렇게 안해도 된다.
...
### 15.4.2 The spread
The competition is to predict the spread, not the proportion $p$. However, because we are assuming there are only two parties, we know that the spread is $p−(1−p)=2p−1$. As a result, everything we have done can easily be adapted to an estimate of $2p−1$. Once we have our estimate $\bar X$ and ${\rm \hat{SE}}(\bar X)$, we estimate the spread with $2 \bar X - 1$  and, since we are multiplying by 2, the standard error is $2{\rm \hat{SE}}(\bar X)$. Note that subtracting 1 does not add any variability so it does not affect the standard error.
...
We want to know the probability that the interval $[\bar X -2 {\rm \hat{SE}}(\bar X),\bar X -2 {\rm \hat{SE}}(\bar X)]$ contains the true proportion $p$. First, consider that the start and end of these intervals are random variables: every time we take a sample, they change. To illustrate this, run the Monte Carlo simulation above twice. We use the same parameters as above:

$[\bar X -2 {\rm \hat{SE}}(\bar X),\bar X +2 {\rm \hat{SE}}(\bar X)]$ 이 범위 아닌가?
...
$${\rm Pr}\left( \bar X - 1.96{\rm \hat{SE}}(\bar X) \leq p \leq \bar X + 1.96{\rm \hat{SE}}(\bar X) \right)$$

By subtracting and dividing the same quantities in all parts of the equation, we get that the above is equivalent to:

$${\rm Pr}\left( -1.96 \leq {{\bar X - p}\over{\rm \hat{SE}}(\bar X)} \leq 1.96 \right)$$
...
$${\rm Pr} \left( -.196 \leq Z \leq 1.96 \right )$$
...
polls <- polls_us_election_2016 %>% 
  group_by(pollster) %>%
  filter(enddate == max(enddate) & state == "U.S.")  %>%
  ungroup() %>%
  mutate(x_hat = rawpoll_clinton / 100, 
         se_hat = sqrt(x_hat * (1 - x_hat) / samplesize),
         ci_low = x_hat - 1.96 * se_hat,
         ci_high = x_hat + 1.96 * se_hat,
         p = mean(x_hat),
         inside = ifelse(p >= ci_low & p <= ci_high, TRUE, FALSE)) %>%
  select(pollster, enddate, x_hat, se_hat, ci_low, ci_high, inside)

# geom_error_barh 이용해보자.
polls %>% ggplot(aes(x=x_hat, y = pollster)) +
  geom_errorbarh(mapping=aes(xmin =ci_high, xmax = ci_low, color = inside)) +
  geom_point(aes(x= x_hat, color = inside)) +
  geom_vline(xintercept = mean(polls$x_hat))
```
...
# [1] "matrix"
# dim(inside)
# [1] 2 10000     2행 10000열 -> ggplot에서 못쓴다. t() 함수로 바꿔줘야 한다.
...
d <- t(inside) %>% 
  ...
  mutate(ci_low = x_hat - 1.96*se_hat, 
         ci_high = x_hat + 1.96*se_hat, 
         inside = ifelse(p <= ci_high & p >= ci_low , 
                         "Yes", "No" ), 
         n = seq(1, B, by=1))

# nrow(d)
# [1] d 행 갯수. 이것 이용하는 것도 방법

d <- d %>% mutate(inside = as.factor(inside))

d[1:100,] %>% 
  ggplot(aes(x=x_hat, y = n)) +
  geom_errorbarh(mapping=aes(xmin = ci_low, xmax = ci_high, color = inside)) +
  geom_point(aes(x= x_hat, color = inside)) +
  geom_vline(xintercept = mean(d$x_hat)) +
  labs(y = "polls") +
  theme_bw()

# table(d$inside) B:10000개일때
#   No  Yes 
#  533 9467 
```

```{r}
# dslab heights data에서, 몇 명을 샘플링을 하느냐에 따라 모집단의 SD 에 가까운지, 치우침에 가까운지, 몬테카를로를 이용해서 보여라
# 주의점, male, female에 차이가 있을 수 있다. male only, female only, 합쳐서 3 개 해보자.
# SD 의 사이즈에 따른 변화도 그래프로 그리기.
...
monte_h <- replicate(B, {
  x <- sample(h$height, size = n, replace = TRUE)
  x_hat <- mean(x)
  se_hat <- sd(x)/sqrt(n)
  return(c(x_hat, se_hat))
}) %>%
  ...
  mutate(ci_low = x_hat - 1.96 * se_hat,
         ci_high = x_hat + 1.96 * se_hat,
         inside = ifelse(m >= ci_low & m <= ci_high, TRUE, FALSE),
         n = seq(1, B))
...
monte_h_m <- replicate(B, {
  x <- sample(h_m$height, size = n, replace = TRUE)
  x_hat <- mean(x)
  se_hat <- sd(x)/sqrt(n)
  return(c(x_hat, se_hat))
}) %>%
  ...
  mutate(ci_low = x_hat - 1.96 * se_hat,
         ci_high = x_hat + 1.96 * se_hat,
         inside = ifelse(m_m >= ci_low & m_m <= ci_high, TRUE, FALSE),
         n = seq(1, B))
...
monte_h_f <- replicate(B, {
  x <- sample(h_f$height, size = n, replace = TRUE)
  x_hat <- mean(x)
  se_hat <- sd(x)/sqrt(n)
  return(c(x_hat, se_hat))
}) %>%
  ...
  mutate(ci_low = x_hat - 1.96 * se_hat,
         ci_high = x_hat + 1.96 * se_hat,
         inside = ifelse(m_f >= ci_low & m_f <= ci_high, TRUE, FALSE),
         n = seq(1, B))
...
polls <- polls %>% mutate(se_hat = sqrt(x_hat*(1-x_hat) / samplesize))
polls <- polls %>% mutate(lower = x_hat - 1.96*se_hat)
polls <- polls %>% mutate(upper = x_hat + 1.96*se_hat)

poll <- select(polls, "pollster", 'enddate', 'x_hat', 'lower', 'upper')
```
...
```{r}
p <- 0.482
poll$hit <- ifelse(poll$upper > p & poll$lower < p, 1, 0)
poll <- as_tibble(poll)
```
...
```
  ...
  difference 의 무엇을 구하라는것인지 이해를 못하겠음. 아래의 풀이가 맞는지 모르겠음

```{r}
N <- polls$samplesize[1]
d_hat <- polls$d_hat[1]
se_d_hat <- sqrt(d_hat*(1-d_hat) / N)
c(d_hat - 1.96*se_d_hat, d_hat + 1.96*se_d_hat)
```
...
poll <- polls %>% 
  mutate(d_hat = rawpoll_clinton / 100 - rawpoll_trump / 100) %>% 
  mutate(se_hat = sqrt(abs(d_hat*(1-d_hat) / samplesize))) %>% 
  mutate(lower = d_hat - 1.96*se_hat) %>% 
  mutate(upper = d_hat + 1.96*se_hat)

poll <- select(poll, "pollster", 'enddate', 'd_hat', 'lower', 'upper')

d <- 0.021
poll$hit <- ifelse(poll$upper > d & poll$lower < d, 1, 0)
poll <- as_tibble(poll)
```

> d 값이 음수가 나올 수 있어서, 절대값을 취했는데, 이것이 맞는지 모르겠다.
...
$$
{\rm Pr} \left( \sqrt N {{\bar X - 0.5}\over{\sqrt{0.5(1-0.5)}}} > \sqrt N {{0.2}\over{\sqrt{0.5(1-0.5)}}} \right )
$$ 
...
A 2014 PNAS paper[^52] analyzed success rates from funding agencies in the Netherlands and concluded that their:
...
R.A. Fisher[^53] was one of the first to formalize hypothesis testing. The “Lady Tasting Tea” is one of the most famous examples.
...
In this case, the odds ratio is ${{a/c}\over{b/d}}$ which is equivalent to $(ad)/(bc)$
...
$${\rm log} \left( {ad \over bc} \right) \pm 1.96 \sqrt{1/a + 1/b + 1/c + 1/d}$$ 
...
Note that the log odds ratio is not defined if any of the cells of the two-by-two table is 0. This is because if $a$, $b$, $c$, or $d$ is 0, the ${\rm log}({{ad}\over {bc}})$ is either the log of 0 or has a 0 in the denominator. For this situation, it is common practice to avoid 0s by adding 0.5 to each cell. This is referred to as the Haldane–Anscombe correction and has been shown, both in practice and theory, to work well.
