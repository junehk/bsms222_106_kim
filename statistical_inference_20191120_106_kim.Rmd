...
N <- polls$samplesize[1]
x_hat <- polls$rawpoll_clinton[1]/100
polls$d_hat[1] + c(-1.96, 1.96) * 2 * sqrt(x_hat * (1 - x_hat) / N)
```
...
polls <- polls %>% mutate(x_hat = rawpoll_clinton/100)
polls <- polls %>% mutate(se_hat = sqrt(x_hat*(1-x_hat) / samplesize))
polls <- polls %>% mutate(d_lower = d_hat - 1.96*2*se_hat / samplesize)
polls <- polls %>% mutate(d_upper = d_hat + 1.96*2*se_hat / samplesize)

poll <- select(polls, "pollster", 'enddate', 'x_hat', 'd_hat', 'd_lower', 'd_upper')

d <- 0.021
poll$d_hit <- ifelse(poll$d_upper > d & poll$d_lower < d, 1, 0)
poll <- as_tibble(poll)
```
...
```{r}
sum(poll$d_hit)/length(poll$d_hit)
```
