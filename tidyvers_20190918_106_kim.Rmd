---
title: "tidyverse_20190918_106_kim"
output:
  html_document:
    df_print: paged
---
...
In the call to `select`, the first argument `murders` is an objects, but `state`, `region`, and `rate` are variable names.
...
answer:
```{r}
# How the rank column is defined
# murders <- mutate(murders, rate = total / population * 10^5)
# murders <- mutate(murders, rank = rank(-rate))

filter(murders, 5 >= rank)
```
...
> `%in%`에 대하여  
> `Vector_A %in% Vector_B` 라고 할 때, A의 값이 B에 있나 확인하는 함수.  
> `c("Boston", "Dakota", "Washington") %in% murders$state`  
> `#> [1] FALSE FASLSE TRUE`  
> `murders$state %in% c("Boston", "Dakota", "Washington")`의 경우 state column에 있는 모든 값들에 T,F 를 실시함. 총 51개 값이 나옴.
...
answer:
```{r}
# How the rate column is defined
# murders <- mutate(murders, rate = total / population * 10^5)

my_table <- filter(murders, region %in% c("Northeast", "West") & 1 > rate)
select(my_table, state, rate, rank)
```
...
$$ \rm{orignial ~~ data} \rightarrow \rm{select} \rightarrow \rm{filter}$$
...
> `mutate` data frame을 첫번째 argument로 가져오고, name 과 변수의 값을 두번째 argument로 받는다.

my answer:
```{r}
# to clear all the mutated data, I applyed the code below:
# data('murders')
...
my_state <- murders %>%
  ...
  filter(rate < 1 & region %in% c("Northest", "West")) %>%
  select(state)
