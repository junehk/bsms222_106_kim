...
> `mutate(state = reorder(state, murder_rate))` 이 코드를 넣어서 순서를 변경  

> geom_bar() 안에 반드시 stat='identity'를 입력해주어야 한다는 것입니다.
stat은 ‘통계’의 뜻을 가진 ‘Statistic’의 약자로, bar 그래프의 형태에 대해 지정해줍니다. identity는 y축 값의 높이를 데이터를 기반으로 정해줄 때 사용해줍니다. 즉, `stat='identity'`는 y축의 높이를 데이터의 값으로 하는 bar그래프의 형태로 지정한다는 것입니다. [출처](https://m.blog.naver.com/PostView.nhn?blogId=coder1252&logNo=220931268317&proxyReferer=https%3A%2F%2Fwww.google.com%2F)
...
> 위쪽은 문제가 많다. 이텔릭체로 되어있는 부분들 모두 다 문제점
...
gapminder %>%
  ...
  labs(y= "Income per day (US Dollar)", x="Continent", title = "Income comparison : 1970 vs 2010") +
...
> `paste0` 코드 : *Concatenate* vectors after converting to character.  
> 사용처  
paste (..., sep = " ", collapse = NULL)  
paste0(..., collapse = NULL)  

> `spread()` 함수 : ‘long’ 형식에서 중간 혹은 ‘wide’ 형식으로 [링크참조](https://statkclee.github.io/r-novice-gapminder/14-tidyr-kr.html)
...
```{r}
dat <- mutate(dat, state = reorder(state,rate))

head(levels(dat$state))
head(dat$state[order(dat$rate)])
#level이 조절된 것을 볼 수 있다.
```
...
```{r}
#summarize() 부분을 없애고 확인해보자. Boxplot은 outlier를 확인시켜줄 것이다.
...
#  summarize(avg = mean(rate)) %>%
  ...
  geom_jitter(pch=22, alpha=0.8, width =0.2) +
  labs(x = "Region", y="Murder Rate") +
...
my_answer : C (south 지역의 outlier들을 보면 알 수 있다.)
...
murders %>% mutate(rate = total/population*100000) %>%
  ...
  geom_boxplot(outlier.shape = NA, aes(color = region)) +
  geom_jitter(pch=22, alpha=0.8, width =0.2, aes(fill = region)) +
  labs(x = "Region", y="Murder Rate") +
  theme(legend.position = "none")
```
...
51개 주 모두를 보기엔 그래프가 이쁘지 않아, 지역별로 묶었다.
```{r}
# 지역으로 묶기.
smallpox <- us_contagious_diseases %>%
  filter(disease == "Smallpox" & weeks_reporting >10 & !is.na(population))
...
Northwest <- murders %>%
  filter(region == "Northeast") %>% select(state) %>% pull()
South <- murders %>%
  filter(region == "South") %>% select(state) %>% pull()
NorthCentral <- murders %>%
  filter(region == "North Central") %>% select(state) %>% pull()
West <- murders %>%
  filter(region == "West") %>% select(state) %>% pull()
...
smallpox %>%
  ...
  ggplot(aes (year, count, group = region) ) +
  geom_line(aes(color = region), alpha = 0.8) +
  facet_grid(~region)
```
...
us_contagious_diseases %>%
  ...
  mutate(year = as.integer(year)) %>%
  ggplot(aes(year, count, group = disease)) +
  geom_line(aes(year, count,color = disease))
```
...
```{r}
dat_s <- us_contagious_diseases %>%
  filter(disease == "Smallpox" & weeks_reporting >10) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>%
  mutate(state = reorder(state, rate))

dat_s <- mutate(as_tibble(dat_s))

dat_s %>%
  filter(!is.na(rate)) %>%
  ggplot(aes(year, rate, group = state)) +
  geom_line(alpha = 0.3) +
  ylab("Smallpox per 10000") +
...
dat_s %>%
  ...
  geom_line() +
  geom_vline(xintercept = 1940, col = "blue") +
  labs(y="Smallpox per 10000", title = "Smallpox in California")
```
...
dat_s %>%
  filter(!is.na(rate)) %>%
  ggplot() +
  geom_line(aes(year, rate, group = state), color = "grey50", alpha = 0.3) +
  geom_line(mapping = aes(year, us_rate), data = avg_s[1:21,], size = 1, color = "black") +
  scale_y_continuous(trans = "sqrt", breaks = c(1, 5, 20)) +
  ggtitle("Smallpox per 10,000 by state") +
  ylab("")+
  xlab("")+
  geom_text(data = data.frame(x=1935, y= 1),
            mapping = aes(x,y, label="US average"),
            color = "black") +
  geom_vline(xintercept = 1940, col ="blue")
```
