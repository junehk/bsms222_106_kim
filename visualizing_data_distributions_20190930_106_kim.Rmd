...
$${\rm Pr} (a < x<b) = \int ^b _ a {1 \over {\sqrt{2\pi}s}}e^{-{1 \over 2}({z-m \over s})^2}dx$$
...
The inverse function $\Phi^{-1}(x)$ gives us the theoretical quantiles for the normal distribution. So, for example, $\Phi^{-1}(x) = 1.96$. In R, we can evaluate the inverse of $\Phi$ using the qnorm function.
...
```{r}
p = seq(0.1, 1, 0.1 )
male_percentile = male %>% quantile(p)
female_percentile = female %>% quantile(p)
```
...
# pnorm function
pnorm(79, mean = mean(x), sd = sd(x))
pnorm(81, mean = mean(x), sd = sd(x))

#compare values
pnorm(79, mean = mean(x), sd = sd(x)) - mean(x <79)
pnorm(81, mean = mean(x), sd = sd(x)) - mean(x <= 81)
```
...
```{r}
p = 1 - pnorm(80, mean = 69, sd = 3)
prop80 <- 10^9 * p

# 전 세계 기준으로 80inches 이상 키의 비율
1- pnorm(80, mean = 69, sd = 3)

# 전 세계 80inches 를 넘는 사람들 중 NBA에 있는 사람들에 대한 비율
150 / prop80
```
...
> `stat = "identity"` 옵션이 뭘까?
[stat = "identity](https://m.blog.naver.com/PostView.nhn?blogId=coder1252&logNo=220931268317&proxyReferer=https%3A%2F%2Fwww.google.com%2F) 여기 참고하면 답이 나옴.
...
> [ggplot 히스토그램 색상넣기](http://www.sthda.com/english/wiki/ggplot2-histogram-plot-quick-start-guide-r-software-and-data-visualization)를 참고함
