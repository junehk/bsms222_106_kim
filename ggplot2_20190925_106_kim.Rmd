...
<img src="https://rafalab.github.io/dsbook/book_files/figure-html/ggplot-example-plot-1.png" width="500">
...
![img2](https://rafalab.github.io/dsbook/R/img/ggplot2-cheatsheeta.png)
![img3](https://rafalab.github.io/dsbook/R/img/ggplot2-cheatsheetb.png)
...
x
 ...
 . So in our plot it’s a line with slope 1 and intercept  
log
(
r
)
...
#  geom_point() +
#  geom_text(label = abb)
# geom_text 안에 aes 함수로 데이터와 ggplot을 연결해주어야한다.
murders %>% ggplot(aes(population, total)) +
  geom_point() +
...
answer:
```{r}
# Solution :geom_label에 color blue를 넣었고, nudge_y를 넣어서 plot찍힌것이랑 label이 겹치지 않게 했다.
murders %>% ggplot(aes(population, total)) +
  geom_point(size = 3) +
  geom_label(aes(label = abb), nudge_y = 70, color ="blue")
# geom_point에 color blue를 넣으면 plot만 파랗게 된다.
```
...
12. Now suppose we want to use color to represent the different regions. In this case which of the following is most appropriate:
  ...
  - C. Use the color argument in 1ggplot1.

  - D. Because we want all colors to be blue, we do not need to map colors, just use the color argument in 1geom_label1.
...
murders %>% ggplot(aes(population, total)) +
  geom_point(aes(col = region), size = 3) +
  geom_label(aes(label = abb), size = 3, nudge_y = 70)
```
...
murders %>% 
  ...
  scale_x_log10() +
  scale_y_log10() +
  ggtitle("Gun murder data 2010 US")
```
