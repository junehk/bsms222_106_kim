...
> `%in%` 복습
> `vector_A %in% vector_B` 이렇게 있다면, A에 있는 것들이 B에 있는지 T/F 확인한다.
```{r eval = FALSE}
data(murders)
murders$region %in% c("West", "Northeast")
#>  [1] FALSE  TRUE  TRUE FALSE  TRUE  TRUE  TRUE FALSE FALSE FALSE
#> [11] FALSE  TRUE  TRUE FALSE FALSE FALSE FALSE FALSE FALSE  TRUE
#> [21] FALSE  TRUE FALSE FALSE FALSE FALSE  TRUE FALSE  TRUE  TRUE
#> [31]  TRUE  TRUE  TRUE FALSE FALSE FALSE FALSE  TRUE  TRUE  TRUE
#> [41] FALSE FALSE FALSE FALSE  TRUE  TRUE FALSE  TRUE FALSE FALSE
#> [51]  TRUE
c("West", "Northeast") %in% murders$region
#> [1] TRUE TRUE
```
...
> 데이터 중 값이 없는 것이 있나보다. 그래서 !is.na(fertility) 를 넣은 것 같다. 확인해볼까?
```{r}
is.na(gapminder$fertility) %>%
  sum()
# Sount Korea 에서 NA 값은 어떨까?
filter(gapminder, country %in% "South Korea") %>%
  select(fertility) %>%
  is.na() %>%
  sum()
```
...
> grouping 을 하기 위해 `ggplot()`안에 `group`구문을 이용해도 되지만, `color`를 이용하면 **자동으로** grouping이 된다.
...
> `label`이라는 data frame을 만들고, 그것을 그래프에 따로 추가하는 형식이다. `geom_text` 구문 안에 data를 로드하고 이름을 붙인 것을 볼 수 있다.  
> 추가로 ggplot layer에 `theme(legend.position = "none")`을 쓰워 legends를 제거했다.
...
In general, we do not recommend using the natural log for data exploration and visualization. This is because while $2^2 , 2^3 , 2^4, \cdots$ or $10^2, 10^3, \cdots$ are easy to compute in our heads, the same is not true for $e^2, e^3, \cdots$, so the scale is not intuitive or easy to interpret.
...
```{r}
filter(gapminder, year == past_year) %>% # past_year = 1970
...
> bump : 튁 튀어나온 고점을 지칭한다. mode가 뭐지? 잘 정리해보자.
...
> 3번째 줄에 `mutate`이용해서 `reorder`를 넣은 것을 보자  
> `FUN = median`이 뭐지?
...
p <- gapminder %>% 
  ...
  xlab("")
#xlab 레이블을 넣어서 아래쪽에 group이라는 것이 안나옴.
p 
```
...
> 1970년대 이후, 새로 새워진 나라(소련 분할)때문에, 2010년에 나라의 총 count가 늘어났다. 1970년대부터 존재했던 나라들만 비교하고싶다.
...
gapminder %>% 
  ...
  ggplot(aes(group, dollars_per_day)) +
  geom_boxplot() + #위는 히스토그램. 여기는 박스플롯
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_y_continuous(trans = "log2") +
  xlab("") +
  facet_grid(. ~ year) # west 랑 비교한다는 것 빠짐.
```
...
gapminder %>% 
  filter(year %in% c(past_year, present_year) & country %in% country_list) %>%
  mutate(year = factor(year)) %>% #year 를 먼저 factor로 한 뒤에 ggplot에 써먹어야 하는 것 같다.
  ggplot(aes(group, dollars_per_day, fill = year)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_y_continuous(trans = "log2") +
  xlab("") 
# facet 은 그래프를 병렬로 두는 것
# ggplot()안에 fill=year로 해서, 한 그래프 안에 담았다.
```
...
> 이 그래프는 1970년대와 2010년대 전체나라의 GDP분포를 나타낸 것. 이 그래프만 보면, 전반적으로 나라들이 잘 살게 되었다고 볼 수 있다. 하지만 서양국가와, 개도국을 개별로 나누어 보면 의미가 달라질 수 있다.
...
> data frame 전반적 구조를 보고 싶으면 str()  
> data frame 안의 한 column을 보고 싶으면 table()이용
...
> 위 그래프에서 제기된 문제점 대로, 전반적 나라의 GDP가 증가했다곤 하지만, 이것이 inflation 때문일 가능성을 제시한다. 전반적인 부의 차이가 크게 나지 않은 것 같기 때문.
...
> `ungroup` 이건 뭘까? `group_by` 했던 것을 이후에 해제하는 함수. pipe line 쓰면서 원하는 부분에만 `group_by`를 쓰기 위해 사용함.  
> `weight` 항목은 왜 만든거지?
...
$$f(p)= {\rm log}\left( {p \over{1-p}} \right) $$
...
![교재의 그림](https://rafalab.github.io/dsbook/book_files/figure-html/ecological-fallacy-all-data-1.png)
