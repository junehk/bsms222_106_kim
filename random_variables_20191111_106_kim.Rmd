...
Before we continue, let’s make an important distinction and connection between the distribution of a list of numbers and a probability distribution. In the visualization chapter, we described how any list of numbers $x_1,\cdots,x_n$ has a distribution. The definition is quite straightforward. We define $F(a)$ as the function that tells us what proportion of the list is less than or equal to $a$. Because they are useful summaries when the distribution is approximately normal, we define the average and standard deviation. These are defined with a straightforward operation of the vector containing the list of numbers `x`:
...
$$\rm number~of~draws ~×~ average~ of~ the~ numbers~ in~ the~ urn$$
...
$$\rm \sqrt{number~ of~ draws } ~×~ standard~ deviation~ of~ the~ numbers~ in~ the~ urn$$
...
$$\mu = {1 \over n}\sum^n_{i=1}x_i$$
$$\sigma = \sqrt{{1 \over n}\sum^n_{i=1}(x_i-\mu)^2}$$
...
This is because the `sd` function R does not return the `sd` of the list, but rather uses a formula that estimates standard deviations of a population from a random sample  $ X_1, \cdots, X_N$ which, for reasons not discussed here, divide the sum of squares by the $N−1$.

$$ \bar{X} = {1 \over N} \sum^N _{i=1} X_i, ~ s= \sqrt{{1 \over {N-1}}\sum^N _{i=1}(X_i - \bar{X})^2}$$
...
$$E[X_1+X_2+⋯+X_n]=E[X_1]+E[X_2]+⋯+E[Xn_]$$ 
...
$$
\begin{aligned}
\rm Pr(Green) &= \rm {{2 ~Green} \over {18~Reds~+~18~Blacks~+~2~Green}} \\
&= {2 \over 40} \\
&= {1 \over 20}
\end{aligned}
$$
...
```{r}
# Answer
x <- sample(c(17,0),1,prob=c(1/20,19/20))
x
```
...
$$
\begin{aligned}
\rm E[X] &= 17 \times {1 \over 20} + 0 \times {19 \over 20} \\
&= {17 \over 20} \\
&= 0.85
\end{aligned}
$$
...
```{r}
B <- 10^6
x <- sample(c(17,0),B,replace=TRUE,prob=c(1/20,19/20))
mean(x)
```
...
\begin{aligned}
\rm SE[X]
&= \rm \sqrt{number~of~draws}~\times~{standard~deviation~of~the~numbers~in~the~urn} \\
&= 1~\times~ \left (|b-a|\sqrt{p(1-p)} \right ) \\
&= 1~\times~ \left( |17-0|~\times~\sqrt{{1\over 20} \times {19 \over 20}} \right) \\
&= 1~\times~ \left( {17 {\sqrt {19}} \over 20} \right)\\
&= 3.705...
\end{aligned}
$$
...
set.seed(1)
n <- 1000
s <- sample(c(17,0), n, replace=TRUE, prob=c(1/20,19/20)) %>% sum()
```
...
B <- 10000
roulette_winnings <- function(n){
  X <- sample(c(17,0), n, replace=TRUE, prob=c(1/20,19/20))
  sum(X)
}
S <- replicate(B, roulette_winnings(n))
...
with $Z$ a standard normal random variable as shown earlier. If we define $μ$ and $σ$  to be the expected value and standard deviation of the urn, respectively (that is of a single loan), using the formulas above we have: $E[S]=nμ$ and $SE[S]=\sqrt{nσ}$. So if we define $z$ =`qnorm(0.01)`, we have:
$$
- {n\mu \over \sqrt{\mu}\sigma}
= {\sqrt{\mu}\sigma \over \sigma}
= z
$$
...
$$
SE[(X_1+X_1+⋯+X1_)/n]=SE[nX_1/n]=σ>σ/\sqrt n
$$ 
...
6. (Harder) What should the interest rate be so that the chance of losing money is 1 in 20? In math notation, what should the interest rate be so that $\rm Pr(S<0)=0.05$?
