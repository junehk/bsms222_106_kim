...
```{r}
# Answer
x <- sample(c(17,-1),1,prob=c(1/20,19/20))
x
```
...
$$
\begin{aligned}
\rm E[X] &= 17 \times {1 \over 20} + (-1) \times {19 \over 20} \\
&= {17+(-19) \over 20} \\
&= -0.1
\end{aligned}
$$
...
```{r}
B <- 10^6
x <- sample(c(17,-1),B,replace=TRUE,prob=c(1/20,19/20))
mean(x)
```
...
&= 1~\times~ \left( |17-(-1)|~\times~\sqrt{{1\over 20} \times {19 \over 20}} \right) \\
&= 1~\times~ \left( {18 {\sqrt {19}} \over 20} \right)\\
&= 3.923...
\end{aligned}
$$
```{r}
...
set.seed(1)
n <- 1000
s <- sum(sample(c(17,-1), n, replace=TRUE, prob=c(1/20,19/20)))
```
...
B <- 10000
roulette_winnings <- function(n){
  X <- sample(c(17,-1), n, replace=TRUE, prob=c(1/20,19/20))
  sum(X)
}
S <- replicate(B, roulette_winnings(n))

# probability for not loosing money
mean(S>0)
```
